Comparison_Metrics <- function(){
  ...
  for ( d in 1:length(disease_pools))
  {
    ...
    for (i in 1:length(broadenrich_results_shredded$GSM1847178$Geneset.ID))
    {
      
      if (broadenrich_results_shredded$GSM1847178$Geneset.ID[i] == j)
      {
        p_values[[x]]<- broadenrich_results_shredded$GSM1847178$P.value[i]
        x <- x+1
    ...
    for (i in 1:length(broadenrich_results_shredded$GSM1847178$Geneset.ID))
    {
      for (j in eval(parse(text=disease_pools[d])))
      {
        if (broadenrich_results_shredded$GSM1847178$Geneset.ID[i] == j && broadenrich_results_shredded$GSM1847178$P.value[i] <= 0.05)
        {
    ...
    greater_than_0.05 <- broadenrich_results_shredded$GSM1847178[which(broadenrich_results_shredded$GSM1847178$P.value > 0.05),]
    less_than_0.05 <- broadenrich_results_shredded$GSM1847178[which(broadenrich_results_shredded$GSM1847178$P.value <= 0.05),]
    true_negatives_ids <- setdiff(greater_than_0.05$Geneset.ID, j) ## All ids that are there in the tool result with p > 0.05 and absent in the disease pool.
    
    false_positives1_ids <- intersect(j,greater_than_0.05$Geneset.ID)
    false_positives2_ids <- setdiff(less_than_0.05$Geneset.ID,j)
