doNormalization <- function(data, ref){
	...
	if(identical(ref, data)){
		...
		tmp = as.data.frame(apply(data,2,as.numeric))
		...
		for( i in 1:ncol(data)){
            ...
            tmp = as.data.frame(apply(tmp[,-1],2,as.numeric))
			id  = colnames(data)[i];
			q.tmp = normalize.quantiles(as.matrix(tmp)); 
			rownames(q.tmp) = rownames(tmp)
...
doFisherScore <- function(all.data,gene.list){
	...
	p.tmp <- pnorm(all.data) ;
    for( i in 1:length(p.tmp)){if(p.tmp[i]>0.5) { p.tmp[i] <- 1-p.tmp[i]}} 
...
iPAScalculation <- function( tumors, reference, gene.list , pathwayName, bgDistributionPath){
	...
	if( nchar (bgDistributionPath ) == 0  ){			
		...
		normal.train.fisher.ref <- c();
        normal.train.bin.ref <- c(); 
        normal.train.GSEA.ref <- c()
	...
	for( i in 1:ncol(tumors)){
		...
		result <- t(as.matrix(c(test.tumor.aveZ.p,test.tumor.fisher.p,  test.tumor.ED.p, test.tumor.GSEA.p) ))#test.tumor.MD.p, ))
...
iPAS = function( all.tumor, all.normal, pathwayListFile, whoami, runMode , bgDistributionPath ){
	...
	gene.list = as.data.frame(gene.list.tmp) 
	colnames(gene.list) = "Gene.Symbol"
